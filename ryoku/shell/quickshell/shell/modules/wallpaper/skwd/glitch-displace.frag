...
// skwd transition: glitch-displace. Ports skwd-daemon's GLITCH_DISPLACE_FRAG
...
vec4 transition(vec2 uv) {
  ...
  vec4 color1 = texture(oldTex, p);
  vec4 color2 = texture(newTex, p);
  vec2 disp = displace(color1, p, 0.33, 0.7, 1.0-ease1(progress));
  vec2 disp2 = displace(color2, p, 0.33, 0.5, ease2(progress));
  vec4 dColor1 = texture(newTex, disp);
  vec4 dColor2 = texture(oldTex, disp2);
